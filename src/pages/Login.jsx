...
function Home() {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        ...
        // Accumulate into ref so we never lose text
      ...
      recognition.onend = () => {
        ...
        console.log("🎙️ Final transcript:", finalTranscriptRef.current.trim());

        // Make sure final text is synced
  ...
  const generatePrep = async () => {
    ...
    try {
      const prompt = `You are a UK university admissions tutor. Generate interview preparation materials as a JSON object with "keyTalkingPoints" (HTML string) and "questions" (array of 5 objects with question, guidance, modelAnswer).
...
Be constructive, specific, and helpful. Use <mark> to highlight key points.`;
  ...
  const translateContent = async (lang) => {
    ...
    try {
      ...
      const prompt = `Translate to ${langMap[lang]}. Keep HTML tags. Separate with "|||": ${q.question}|||${q.guidance}|||${q.modelAnswer}`;
      const payload = { contents: [{ parts: [{ text: prompt }] }] };
