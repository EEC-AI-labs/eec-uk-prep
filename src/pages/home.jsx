...
import { Shield, ChevronLeft, ChevronRight, Mic, RefreshCw, Trash2 } from 'lucide-react';

// Constants
const UNIVERSITIES = ["Anglia Ruskin University (ARU London)", "Aston University (Aston University London)", "Coventry University (Coventry University London / CU London)", "De Montfort University (DMU London)", "Glasgow Caledonian University (GCU London)", "Loughborough University (Loughborough University London)", "Northumbria University (Northumbria University London Campus)", "Nottingham Trent University (NTU London)", "Staffordshire University (Staffordshire University London)", "Teesside University (Teesside University London)", "Ulster University (Ulster University London)", "University of the West of Scotland (UWS London)", "University of Sunderland (University of Sunderland in London)", "University of Wales Trinity Saint David (UWTSD London)", "University of Portsmouth (University of Portsmouth London)", "York St John University (York St John London Campus)", "University of Cambridge", "University of Oxford", "Imperial College London", "University College London (UCL)", "King's College London", "University of Edinburgh", "University of Manchester", "University of Bristol", "University of Warwick", "University of Glasgow", "Durham University", "University of Sheffield", "University of Birmingham", "University of Leeds", "University of Southampton", "University of Nottingham", "University of York", "Lancaster University", "University of Bath", "University of Exeter", "Newcastle University", "Cardiff University", "Queen Mary University of London", "University of Aberdeen", "University of Liverpool", "University of Reading", "University of Sussex", "Royal Holloway, University of London", "SOAS University of London"].sort();

const COURSE_LEVELS = ["Foundation", "International Year 1", "International Year 2", "Bachelors", "Bachelors with Placement Year", "Bachelors Top-Up", "Pre-Masters", "Masters", "Masters with Placement Year"];
...
function Home() {
  ...
  const [modal, setModal] = useState({ show: false, message: '', isConfirm: false, onConfirm: null });
  ...
  const generatePrep = async () => {
    if (!formData.prevQualification || !formData.sponsorOccupation || !formData.careerGoals) {
    ...
    {"question": "...", "guidance": "...", "modelAnswer": "..."}
      ...
      if (result) {
        try {
          ...
          showModal("The AI returned content in an unexpected format. Check error log below.");
        }
      }
    } catch (error) {
...
Be constructive, specific, and helpful. Use <mark> to highlight key points.`;
      ...
      const rawFeedback = await makeApiCall(payload);
      
      if (rawFeedback) {
        const scoreMatch = rawFeedback.match(/<strong>Score:<\/strong>\s*(\d{1,2})/);
  ...
  const translateContent = async (lang) => {
    ...
    try {
      ...
      const question = prepContent.questions[currentQuestionIndex].question;
      const guidance = prepContent.questions[currentQuestionIndex].guidance;
      const modelAnswer = prepContent.questions[currentQuestionIndex].modelAnswer;
      
      const prompt = `Translate to ${langMap[lang]}. Keep HTML tags. Separate with "|||": ${question}|||${guidance}|||${modelAnswer}`;
      
      const payload = { contents: [{ parts: [{ text: prompt }] }] };
      const result = await makeApiCall(payload, translationControllerRef.current.signal);
  ...
  return (
    <div className=" flex flex-col w-full min-h-screen  ">
      ...
      <nav className="w-full max-w-7xl mx-auto px-6">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-bold text-slate-800">UK Pre-CAS Prep by EEC</span>
            </div>
          </div>
        </nav>
      </header>
      ...
      <main className="flex-1 w-full px-4 sm:px-8 py-12 flex justify-center ">
        ...
        {step === 1 && (
          <section className="w-full mb-16 text-center">
            ...
            <p className="mt-4 text-lg text-slate-600 max-w-2xl mx-auto">
              Get hyper-personalized Pre-CAS interview preparation. Anytime, Anywhere.
            </p>
            
            <div className="mt-10 max-w-xl mx-auto bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200">
              <h3 className="text-xl font-bold text-left text-slate-800 mb-6">Step 1: Your Course Details</h3>
              
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Choose UK University</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Choose Course Level</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Write Exact Course Name</label>
                  <input
                    type="text"
                ...
                <button
                  onClick={() => {
                    if (!formData.university || !formData.courseLevel || !formData.course) {
                      showModal('Please complete all fields');
                      return;
                    }
                    setStep(2);
                  }}
                  className="w-full bg-slate-600 text-white font-semibold py-3 px-6 rounded-lg hover:bg-slate-700 transition-all transform hover:scale-[1.02]"
                >
        ...
        {step === 2 && (
          <section className="w-full mb-16 text-center">
            <h1 className="text-3xl font-bold text-slate-900 mb-4">Complete Your Profile</h1>
            
            <div className="mt-10 max-w-xl mx-auto bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200">
              <h3 className="text-xl font-bold text-left text-slate-800 mb-6">Step 2: Your Personal Profile</h3>
              
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Previous Qualification & Grade</label>
                  <input
                    type="text"
                    value={formData.prevQualification}
                    onChange={(e) => handleInputChange('prevQualification', e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Primary Funding Source</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Sponsor's Occupation</label>
                  <input
                    type="text"
                    value={formData.sponsorOccupation}
                    onChange={(e) => handleInputChange('sponsorOccupation', e.target.value)}
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Future Career Goals</label>
                  <textarea
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1 text-left">Gaps in Education/Work (if any)</label>
                  <textarea
                    value={formData.studyGap}
                    onChange={(e) => handleInputChange('studyGap', e.target.value)}
                    rows="2"
                ...
                >
                  {isGenerating ? 'Generating...' : 'Start Ultra-Personalized Prep'}
                </button>
              </div>
            </div>
          </section>
        ...
        {step === 3 && prepContent && (
          <section className="mb-16">
            <div className="max-w-4xl mx-auto">
              {/* Key Talking Points */}
              <div className="mb-12" dangerouslySetInnerHTML={{ __html: prepContent.keyTalkingPoints }} />
              
              <div className="flex justify-end mb-4">
                <button
                  onClick={() => {
                    showModal('Start a new prep session? Current questions will be cleared.', true, () => {
                      setPrepContent(null);
                      setStep(1);
                      hideModal();
                    });
              ...
              <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200">
                ...
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-lg font-bold text-indigo-600">
                    Question {currentQuestionIndex + 1} of {prepContent.questions.length}
                  </h3>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={currentQuestionIndex === prepContent.questions.length - 1}
                      className="p-2 rounded-md hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                {activeTab === 'practice' && (
                  <div>
                    ...
                    {isRecording && (
                      <div className="text-center py-4">
                        ...
                        <p className="text-sm text-slate-500 mt-2">
                          {elapsedSeconds < 60 ? 'Speak for 60-120 seconds' : 'Excellent! You can stop when ready.'}
                        </p>
                    ...
                    {!isRecording && transcript && (
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Review and edit your answer:</label>
                        <textarea
                          value={transcript}
                    ...
                    {feedback && (
                      <div className="mt-6 bg-slate-50 p-6 rounded-xl border border-slate-200">
                        <div className="flex justify-between items-start gap-4">
                          <div>
                            <h4 className="text-lg font-semibold text-slate-800">Your Feedback</h4>
                            <p className="text-sm text-slate-500">{new Date(feedback.timestamp).toLocaleString()}</p>
                          </div>
                          <div className="text-center ml-4 flex-shrink-0">
                            <div className={`w-24 h-24 rounded-full flex items-center justify-center ${
                              feedback.score >= 8 ? 'bg-green-100' : feedback.score >= 5 ? 'bg-yellow-100' : 'bg-red-100'
                            }`}>
                              <p className={`text-4xl font-extrabold ${
                                feedback.score >= 8 ? 'text-green-600' : feedback.score >= 5 ? 'text-yellow-600' : 'text-red-600'
                              }`}>
                                {feedback.score}<span className="text-2xl font-semibold opacity-60">/10</span>
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        />
                        <h5 className="font-semibold mt-6 mb-2 text-slate-700 text-sm">Your Answer Transcript:</h5>
                        <p className="text-sm p-3 bg-white rounded-md border text-slate-600">{feedback.transcript}</p>
                        <button
                          onClick={() => {
                            setTranscript('');
                            setFeedback(null);
              ...
              <div className="mt-12">
                <div className="flex justify-between items-center border-b-2 border-slate-200 pb-3 mb-8">
                  <h2 className="text-2xl sm:text-3xl font-bold text-slate-800">Practice History</h2>
                  <button
                    onClick={() => {
                ...
                {history.length === 0 ? (
                  <p className="text-center text-slate-500 py-8 bg-slate-50 rounded-lg">
                    You haven't practiced any questions yet. Your session history will appear here.
                  </p>
                ) : (
                  <div className="space-y-4">
                    {history.map((item, idx) => (
                      <details key={idx} className="bg-white rounded-lg shadow-md overflow-hidden border border-slate-200">
                        <summary className="flex justify-between items-center p-4 list-none cursor-pointer">
                          <div className="flex-grow pr-4">
                            <p className="font-semibold text-slate-700">{item.question}</p>
                        ...
                        <div className="p-4 border-t border-slate-200 bg-slate-50/70">
                          ...
                          />
                          <h5 className="font-semibold mt-4 mb-1 text-xs text-slate-600 uppercase">Your Answer:</h5>
                          <p className="text-sm text-slate-600">{item.transcript}</p>
                        </div>
                      </details>
                    ))}
                  </div>
